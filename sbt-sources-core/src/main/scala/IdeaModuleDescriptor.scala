...
class IdeaModuleDescriptor(val project: BasicDependencyProject, val log: Logger) extends SaveableXml with ProjectPaths {
  ...
  def content: Node = {
    ...
    <module type="JAVA_MODULE" version="4">
      ...
      <component name="NewModuleRootManager" inherit-compiler-output={env.projectOutputPath.get.isDefined.toString}>
        {if (env.projectOutputPath.get.isEmpty) {
          <output url={"file://$MODULE_DIR$/" +
                project.asInstanceOf[ScalaPaths].mainCompilePath.relativePath.toString}/>
                  <output-test url={"file://$MODULE_DIR$/" +
                project.asInstanceOf[ScalaPaths].testCompilePath.relativePath.toString}/>
      } else {
        scala.xml.Null
      }}<exclude-output/>
        ...
        case _ => xml.Null
          ...
          <orderEntry type="library" name="buildScala" level="project"/>{dependencyProjects.map {
        dep =>
      ...
      def named(pf: PathFinder, suffix: String) = Map() ++
              pf.getFiles.toList.sort(_.getAbsolutePath < _.getAbsolutePath).map(file =>
  ...
  def sourceFolders(folders: (PathFinder, Boolean)*) = {
    val nodes = folders.foldRight(List[Node]()) {
      (folder, a) =>
  ...
  def sourceFolder(path: String, isTestSourceFolder: Boolean) = <sourceFolder url={"file://$MODULE_DIR$/" +
          path} isTestSource={isTestSourceFolder.toString}/>

  def webFacet(webProject: DefaultWebProject): Node = {
    <facet type="web" name="Web">
      <configuration>
        <descriptors>
            <deploymentDescriptor name="web.xml" url={String
                  .format("file://$MODULE_DIR$/%s/WEB-INF/web.xml", relativePath(webProject.webappPath.asFile))}/>
        </descriptors>
        <webroots>
            <root url={String
                  .format("file://$MODULE_DIR$/%s", relativePath(webProject.webappPath.asFile))} relative="/"/>
        </webroots>
      </configuration>
    </facet>
  }

  def moduleLibrary(scope: Option[String], sources: Option[String], javadoc: Option[String], classes: Option[String],
                    relativePaths: Boolean): Node = {
    def root(entry: Option[String]) =
      entry.map {
        e =>
          val url = if (relativePaths) {
            String.format("jar://$MODULE_DIR$/%s!/", e)
          } else {
            e
          }
            <root url={url}/>
