...
class IdeaProjectDescriptor(val project: BasicDependencyProject, val log: Logger) extends ProjectPaths {
  ...
  val vcsName = List("svn", "Git").foldLeft("") {
    (res, vcs) =>
      if (project.path("." + vcs.toLowerCase).exists) {
        vcs
      } else {
        res
      }
  ...
  private def projectModuleManagerComponent: xml.Node =
    <component name="ProjectModuleManager">
      ...
      (childProjects ::: mainModule).map {
        case (modulePath, moduleName) =>
          val pathPrefix = modulePath match {
            case "" => ""
            case p => "/" + p
          }
            <module fileurl={String.format("file://$PROJECT_DIR$%s/%s.iml", pathPrefix, moduleName)} filepath={String
                  .format("$PROJECT_DIR$%s/%s.iml", pathPrefix, moduleName)}/>
      }}
      </modules>
    </component>

  private def project(inner: xml.Node*): xml.Node = <project version="4">
    {inner}
  </project>
    ...
    <component name="libraryTable">
      <library name={libraryName}>
        <CLASSES>
          ...
          <JAVADOC/>{if (includeSources) {
        <SOURCES>
            <root url={String.format("jar://$PROJECT_DIR$/%s!/",
            relativePath((scalaJarDir / "scala-compiler-sources.jar").asFile))}/>
            <root url={String.format("jar://$PROJECT_DIR$/%s!/",
            relativePath((scalaJarDir / "scala-library-sources.jar").asFile))}/>
        </SOURCES>
      } else {
        xml.Null
      }}
  ...
  private def projectRootManagerComponent: xml.Node =
    <component name="ProjectRootManager" version="2" languageLevel={env.javaLanguageLevel
            .value} assert-keyword="true" jdk-15="true" project-jdk-name={env.projectJdkName
            .value} project-jdk-type="JavaSDK">
  ...
  def save {
    ...
    if (projectPath.exists) {
      ...
      if (!configFile("vcs.xml").exists) {
        saveFile(configDir, "vcs.xml", project(vcsComponent))
      }
      ...
      log.error("Skipping .idea creation for " + projectPath + " since directory does not exist")
    }
  }
  ...
  private object miscTransformer extends RuleTransformer(
    new RewriteRule() {
      override def transform(n: Node): Seq[Node] = n match {
        ...
        case e@Elem(_, "component", _, _, _*) if e \ "@name" ==
                Text("ProjectRootManager") => projectRootManagerComponent
        case _ => n
      }
    }
  )
