...
trait ScriptedTestAssertTasks extends Project {
  lazy val assertExpectedXmlFiles = task {
    val expectedFiles = listFiles(info.projectPath.asFile, Seq("expected").toArray, true).toArray
            .map(_.asInstanceOf[File])
    Seq(expectedFiles: _*).map(assertExpectedXml).foldLeft[Option[String]](None) {
      (acc, fileResult) => if (acc.isDefined) {
        acc
      } else {
        fileResult
      }
  ...
  private def assertExpectedXml(expectedFile: File): Option[String] = {
    ...
    if (actualFile.exists) {
      assertExpectedXml(expectedFile, actualFile)
    }
    else {
      Some("Expected file " + actualFile.getAbsolutePath + " does not exist.")
    }
  }
